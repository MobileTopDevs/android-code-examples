...
fun AddGameScreen(
    ...
    val apps by addGameViewModel.appsWithSettings.collectAsStateWithLifecycle()
    val search by addGameViewModel.search.collectAsStateWithLifecycle()
    val saveAppsResponse by addGameViewModel.saveAppsResponse.collectAsStateWithLifecycle()

    LaunchedEffect(key1 = true) {
        lifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
            addGameViewModel.getApps()
        }
    }
    ...
    AddGameContent(
        apps = apps,
        onBack = onBack,
        onSave = addGameViewModel::save,
        search = search,
        onSearchTextChanged = addGameViewModel::updateSearchText,
        onAppChecked = addGameViewModel::preSaveApp
    )
}
                ...
                apps?.let {
                    items(it) { app ->
                        CheckableAppSettingsItem(
                            ...
                            title = app.label,
                            checked = app.settings.visibleForGames,
                            onCheckedChange = { checked ->
                                onAppChecked.invoke(
                                    app.copy(
                                        settings = app.settings.copy(visibleForGames = checked)
                                    )
                                )
                            }
                        )
